...
class PostFixInterpreter {
    ...
    evaluatePostfixExp(inputExp){
        ...
        for(let i=0; i<expTokens.length; i++){
            ...
            if(token.match(/^[A-Z]$/)){
                ...
                if(value === undefined){
                    console.log(`\nSymbol "${token}" not found, nothing pushed 0\n`);
                    this.stack.push(0);
                }else{
                ...
                switch(token){
                    ...
                    //for this case if the addition operator has been inserted then addition
                    case '+':
                    ...
                    case '^':
                        this.stack.push(operand2^ operand1);
                        console.log(`\n\nPerformed exponentiation: ${operand2} ^ ${operand1} = ${operand2 ^ operand1}\n\n`);
                        break;    
            ...
            //this checks if the user only requests to update a value
                ...
                if(!isNaN(num) && this.isValidNumber(num)){
                    this.stack.push(num);
                    console.log(`Pushed number: ${num}\n`);

                }else{

                    //this verifies if the nummber has been inserted is between -100 and 100 value range
    ...
    updateSymbolTable(symbol, value){
        ...
        if(/^[A-Z]$/.test(symbol)){
            ...
            //thsi verifies if the value that was inserted is between -100 and 100 by calling the function
            if(this.isValidNumber(value)){
                ...
                if(this.symbolCounter < 1000){
                    ...
                    console.log(`The symbol table is full, the current number of sumbols in the table are "  ${this.symbolCounter} "`);
                }
            }
            else{
    ...
    searchSymbolHash(symbol){
        if(this.collection.hasOwnProperty(symbol)){
            console.log("\nSearch status: \nSymbol has been found successfull.");
    ...
    //this function verifies whether the input from the user for variable values are -100 and 100 range
    isValidNumber(num){
...
function getUserInput(){
     ...
     userInput.question('Please enter your selection here: ', (selection) => {
        ...
        switch (selection) {
            ...
            case '2':
                console.log(`\nSymbol table: ${JSON.stringify(interpreter.collection)} \n\n`);
                getUserInput();
                break;
